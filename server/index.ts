...
app.get('/api/health', (_req, res) => {
  res.json({
    ...
    paperSlippageEnabled: process.env.PAPER_SLIPPAGE_ENABLED !== 'false',
...
app.get('/api/competition/leaderboard/:id', async (req, res) => {
  try {
    ...
    // Railway : le loop 10s + les fills gardent le ranking frais.
    // Un GET ne doit plus recalculer tout le roster ni réécrire Neon.
