import type { TranslationResources } from './en';

const CONTACT_EMAIL = 'breakout.pro.tv@gmail.com';

export const fr: TranslationResources = {
  common: {
    cancel: 'Annuler',
    close: 'Fermer',
    back: 'Retour',
    validate: 'Valider',
    loading: 'Chargement...',
    refresh: 'Actualiser',
    unknownError: 'Erreur inconnue',
    settings: 'Réglages',
    leaderboard: 'Classement',
  },

  share: {
    appName: 'BTFARENA.COM',
    eyebrow: 'Partage ton exploit',
    cta: 'Partager',
    titleRank: 'Partager mon classement',
    titleTrade: 'Partager ce trade',
    download: 'Télécharger',
    shareBtn: 'Partager',
    generating: 'Génération de l’image…',
    error: 'Impossible de générer l’image.',
    shared: 'Partagé !',
    previewAlt: 'Aperçu de la carte de partage BTF',
    shareTextRank: 'Je suis #{{rank}} sur {{context}} — BTFARENA.COM',
    shareTextTrade: 'Mon trade {{pair}} sur BTFARENA.COM',
  },

  seo: {
    homeTitle: 'BTF Arena : compétition de trading en ligne 100% gratuite & lots à gagner',
    homeDesc:
      'BTF Arena, la compétition de trading en ligne 100% gratuite : crypto, forex, indices, actions et matières premières. Affronte des traders du monde entier, grimpe au classement en temps réel et gagne des lots exclusifs. Inscription gratuite, sans dépôt.',
    homeKeywords:
      'BTF Arena, compétition de trading, compétition de trading en ligne, trading gratuit, tournoi de trading, lots à gagner, concours de trading, paper trading, trading forex, trading indices, trading actions, trading matières premières',
    leaderboardTitle: 'Classement global des traders',
    leaderboardDesc:
      'Découvre les meilleurs traders de BTF Arena. Classement global en temps réel des compétitions de trading en ligne 100% gratuites.',
    arenaDesc:
      'Classement en temps réel de l’arène « {{title}} » sur BTF Arena — compétition de trading en ligne 100% gratuite. Suis les meilleurs traders et les lots à gagner.',
    bonusTitle: 'Live Trading Bonus — Deals broker & exchange',
    bonusDesc:
      'Bonus exclusifs et codes promo pour nos brokers et exchanges partenaires. Récompenses de dépôt, frais réduits et offres spéciales pour les traders de BTF Arena.',
    newsTitle: 'Actualités BTF Arena',
    newsDesc: 'Annonces officielles, nouvelles arènes et infos de la ligue BTF Arena.',
  },

  lang: {
    label: 'Langue',
    en: 'EN',
    fr: 'FR',
  },

  header: {
    arenas: 'Arènes',
    play: 'Jouer',
    live: 'Live',
    trade: 'Trader',
    profile: 'Profil',
    settings: 'Réglages',
    payouts: 'Payout',
    logout: 'Déconnexion',
    login: 'Connexion',
    menu: 'Menu',
    account: 'Compte',
    discover: 'Découvrir',
    news: 'Actualités',
    unreadNews: 'Actualité non lue',
  },

  hero: {
    eyebrow: 'BTF Arena · Compétition de trading en ligne 100% gratuite',
    seoHeading: 'BTF Arena — Compétition de trading en ligne gratuite',
    ctaArenas: 'Voir les arènes',
    ctaHow: 'Comment ça marche',
    subtitle: 'Inscris-toi à une arène, trade sur notre plateforme et gagne des prix.',
  },

  spotlight: {
    kicker: 'Arène à la une',
    startsIn: 'Commence dans',
    endsIn: 'Se termine dans',
    traders: 'Traders inscrits',
    watch: 'Regarder le direct',
    join: 'Rejoindre',
  },

  livePage: {
    seoTitle: 'Arènes live — BTF Arena',
    seoDesc: 'Suis la compétition BTF Arena en direct, consulte la PnL Race et accède au classement de chaque arène.',
    kicker: 'BTF Arena · En direct',
    title: 'Le centre live',
    lead: 'Une arène chaque semaine. Suis la course au PnL en temps réel, encourage les traders et rejoins le chat de l’arène.',
    weeklyArena: 'Arène hebdomadaire',
    schedule: 'Rendez-vous chaque semaine',
    scheduleValue: 'Lundi · 08:00 UTC',
    upcoming: 'Prochaines arènes',
    archived: 'Arènes archivées',
    archivedKicker: 'Archive',
    emptyTitle: 'La prochaine arène arrive',
    emptyLead: 'Le nouveau challenge sera annoncé ici.',
  },

  status: {
    live: 'Live',
    registration: 'Inscription',
    startingSoon: 'Bientôt',
    ended: 'Terminée',
  },

  mode: {
    paper: 'Paper',
    real: 'Réel',
  },

  prize: {
    toWin: 'À gagner',
    rewardAlt: 'Récompense',
    moreLots: '+ {{count}} lot',
    moreLots_other: '+ {{count}} lots',
  },

  auth: {
    traderAccess: 'Accès trader',
    titleVerifyPhone: 'Vérification SMS',
    titleVerifyEmail: 'Vérifie ton email',
    titleLogin: 'Se connecter',
    titleSignup: 'Créer un compte',
    subVerifyPhone: 'Dernière étape : saisis le code reçu par SMS pour valider ton numéro.',
    subVerifyEmail: 'Saisis le code à 6 chiffres reçu par email.',
    subRequest: 'Inscription par email + vérification téléphone (anti multi-comptes).',
    tabLogin: 'Connexion',
    tabSignup: 'Inscription',
    email: 'Email',
    emailPlaceholder: 'trader@exemple.com',
    username: 'Pseudo',
    usernamePlaceholder: 'Ton nom de trader',
    phone: 'Téléphone',
    phonePlaceholder: '+33 6 12 34 56 78',
    phoneHint: 'Vérification SMS unique à l\'inscription (anti multi-comptes). Format international.',
    consentText: 'J\'accepte les',
    consentAnd: 'et la',
    consentNewsletter: ', et j\'accepte de recevoir la newsletter et les informations de BTF Arena.',
    sending: 'Envoi du code...',
    getCode: 'Recevoir mon code',
    switchToSignup: 'Pas encore inscrit ? Bascule sur Inscription.',
    switchToLogin: 'Déjà un compte ? Bascule sur Connexion.',
    stepEmail: '1. Email',
    stepSms: '2. SMS',
    codeSentTo: 'Code envoyé à',
    emailSendFailed: 'Envoi email échoué',
    devModeMailer: 'Mode dev (mailer non configuré)',
    codeGeneratedFor: 'Code généré pour',
    backupCode: 'Code de secours',
    verifying: 'Vérification...',
    editEmail: 'Modifier email',
    resendCode: 'Renvoyer un code',
    smsSentTo: 'SMS envoyé au',
    smsSendFailed: 'Envoi SMS échoué',
    devModeTwilio: 'Mode dev (Twilio non configuré)',
    smsNotSent: 'SMS non envoyé, utilise le code ci-dessous',
    confirmAccount: 'Confirmer mon compte',
    smsNotReceived:
      'Tu n\'as pas reçu le SMS ? Vérifie le format du numéro, ou redemande un code email pour redémarrer.',
  },

  authErrors: {
    testLogin: 'Connexion test impossible',
    request: 'Demande impossible',
    verify: 'Vérification impossible',
    verifySms: 'Vérification SMS impossible',
    loginToJoin: 'Connecte-toi pour rejoindre une compétition',
    join: 'Join impossible',
    codeMismatch: 'Le code ne correspond pas à cette compétition',
    tradingAccess: 'Accès trading impossible',
  },

  user: {
    myProfile: 'Mon profil',
    greeting: 'Salut {{name}}',
    totalPnl: 'PnL total',
    avgPnl: 'PnL moy.',
    arenas: 'Arènes',
    winRate: 'Winrate',
    avgRR: 'RR moyen',
    profitFactor: 'Profit factor',
    globalLeaderboard: 'Leaderboard global',
    tradeJournal: 'Mon journal de trading',
    publicProfile: 'Ma page publique',
  },

  journal: {
    eyebrow: 'Mon journal',
    title: 'Journal de trading',
    subtitle: 'Tous tes trades fermés, toutes arènes confondues — courbe d\u2019équité, meilleurs et pires trades.',
    back: 'Retour',
    loading: 'Chargement...',
    empty: 'Aucun trade fermé pour le moment. Rejoins une arène et commence à trader !',
    loginRequired: 'Connecte-toi pour voir ton journal de trading.',
    backToHome: 'Retour à l\u2019accueil',
    equityCurve: 'Courbe d\u2019équité (PnL réalisé)',
    tradesCount: '{{count}} trade fermé',
    tradesCount_other: '{{count}} trades fermés',
    summary: 'Résumé',
    realizedPnl: 'PnL net (frais incl.)',
    totalFees: 'Frais totaux',
    feesCol: 'Frais',
    netPnlCol: 'PnL net',
    wins: 'Gagnants',
    losses: 'Perdants',
    winRate: 'Winrate',
    avgRR: 'RR moyen',
    profitFactor: 'Profit factor',
    maxWinStreak: 'Série gagnante',
    maxLossStreak: 'Série perdante',
    tradesLabel: 'Trades',
    allArenas: 'Toutes les arènes',
    bestTrades: 'Meilleurs trades',
    worstTrades: 'Pires trades',
    allTrades: 'Historique des trades',
    date: 'Date',
    arena: 'Arène',
    pair: 'Paire',
    side: 'Sens',
    leverage: 'Levier',
    long: 'Long',
    short: 'Short',
  },

  globalLeaderboard: {
    title: 'Leaderboard global',
    subtitle: 'Classement par saison et classement all-time',
    seasonBoard: 'Saison en cours',
    allTimeBoard: 'All-time',
    globalTab: 'Global Leaderboard',
    comingSoon: 'Bientôt disponible',
    comingSoonDesc: 'Cette saison n\u2019a pas encore commencé. Le classement sera dévoilé dès son lancement.',
    allTimeTitle: 'Leaderboard global',
    allTimeDesc: 'Stats cumulées sur toutes les arènes et toutes les saisons',
    back: 'Retour',
    rank: 'Rang',
    trader: 'Trader',
    totalPnl: 'PnL total',
    winRate: 'Winrate',
    avgRR: 'RR moyen',
    profitFactor: 'Profit factor',
    trades: 'Trades',
    arenas: 'Arènes',
    empty: 'Aucun participant pour le moment.',
    loading: 'Chargement...',
    noTrades: 'Aucun trade fermé',
    rewardEyebrow: 'Lot principal',
    rewardTitle: 'Le top 5 remporte une invitation au BTF 2027',
    rewardDesc: 'Les 5 meilleurs traders du leaderboard global gagnent une invitation personnelle à l\u2019événement BTF 2027.',
    qualifies: 'BTF 2027',
    prize: {
      eyebrow: '{{season}} · Le grand prix',
      headline: '90 jours pour finir #1 et trader au BTF 2027',
      subtitle: 'Pendant toute la {{season}}, participe à un maximum d’arènes : le classement additionne ton PnL total sur toutes les arènes de la saison. Plus tu performes souvent, plus tu peux prendre la tête et décrocher ta place dans l’arène physique du BTF 2027.',
      badgeLabel: 'Badge de Champion',
      badgeHint: 'Voir le badge en grand',
      shirtLabel: 'Maillot officiel du Champion',
      shirtTag: 'Ton ticket pour l\u2019arène physique BTF 2027',
      arenaLabel: 'Accès Arène BTF 2027',
      arenaDesc: 'Ta place pour trader en live dans l\u2019arène physique à Paris',
      winnerNote: 'Un seul vainqueur par saison — la place est unique.',
      step1: 'Enchaîne les arènes',
      step2: 'Cumule ton PnL total',
      step3: 'Finis #1 après 90 jours',
      cta: 'Voir le badge',
    },
  },

  homeSeason: {
    eyebrow: 'Saison en cours',
    live: 'En direct',
    title: 'La {{season}} est lancée',
    subtitle: 'Une arène par semaine : cumule ton PnL pendant la saison. Le numéro 1 final gagne son invitation pour le Major de Paris.',
    stackTitle: 'Enchaîne les arènes pour grimper',
    stackDesc: 'Ton classement de saison cumule les performances de toutes tes arènes. Plus tu enchaînes d\u2019arènes en tradant proprement, plus tu montes — une seule arène ne suffit pas.',
    prizeBadge: 'Badge Champion',
    prizeShirt: 'Maillot officiel',
    prizeArena: 'Major de Paris',
    tagline: 'Une arène par semaine. Termine numéro 1 de la saison pour gagner ta place au Major de Paris.',
    prizesLabel: 'À remporter',
    daysLeftLabel: 'jours restants',
    lastDayLabel: 'Dernier jour',
    progressLabel: 'Saison écoulée',
    rankReward: 'Le n°1 de la saison est qualifié pour Paris',
    cta: 'Voir le classement de la saison',
    ctaShort: 'Explorer la saison',
    bestTitle: 'Qui sera le meilleur',
    bestTitleEm: 'de la saison ?',
    bestLead: 'Le n°1 du classement de saison est invité au Major de Paris.',
    seeFull: 'Voir le classement complet',
    arenas: '{{count}} arènes',
  },

  seasons: {
    upcoming: 'Bientôt',
    ended: 'Terminée',
    daysLeft: '{{count}} jours restants',
    lastDay: 'Dernier jour !',
    startsIn: 'Débute dans {{count}} jours',
    badgesTitle: 'Badge saison',
    championBadge: '#1 de la saison',
    summer2026: {
      name: 'Summer Season',
      rewardEyebrow: 'Summer Season · Récompenses',
      rewardTitle: 'Le #1 remporte le badge de champion de la saison',
      rewardDesc: 'Le meilleur trader de la Summer Season décroche le badge de champion affiché sur son profil, en plus des lots exclusifs.',
    },
    autumn2026: {
      name: 'Autumn Season',
      rewardEyebrow: 'Autumn Season · Récompenses',
      rewardTitle: 'Le #1 remporte le badge de champion de la saison',
      rewardDesc: 'Le meilleur trader de l\u2019Autumn Season décroche le badge de champion affiché sur son profil, en plus des lots exclusifs.',
    },
  },

  news: {
    navLabel: 'Actualités',
    kicker: 'BTF Arena',
    title: 'Actualités',
    lead: 'Annonces officielles, arènes à venir et infos de la ligue.',
    loading: 'Chargement des actualités…',
    empty: 'Aucune actualité publiée pour le moment.',
    loadError: 'Chargement impossible',
    featured: 'À la une',
    read: 'Lire l’article',
    back: 'Toutes les actualités',
    homeBanner: 'Dernières actualités',
    homeAll: 'Toutes les news',
  },

  bonus: {
    navLabel: 'Trade Live Bonus',
    eyebrow: 'Offres partenaires',
    title: 'Trade Live Bonus',
    subtitle: 'Offres exclusives, bonus et codes promo de nos exchanges et brokers partenaires. Inscris-toi via ces liens pour débloquer les meilleures conditions.',
    disclaimer: 'Ces liens sont des liens d\u2019affiliation. Le trading comporte un risque de perte.',
    homeSub: 'Bonus de dépôt, frais réduits et codes promo exclusifs — uniquement via BTF Arena.',
    homeCta: 'Récupérer mes bonus',
    homeKicker: 'Exclusif traders BTF',
    homeTitleEm: 'Débloque tes bonus',
    homePerkDeposit: 'Bonus de dépôt',
    homePerkFees: 'Frais réduits',
    homePerkCodes: 'Codes promo',
    homeLive: 'Offres live',
    featuredEyebrow: 'Meilleures offres',
    empty: 'Aucun partenaire disponible pour le moment. Reviens bientôt.',
    code: 'Code',
    copyCode: 'Copier le code',
    claim: 'En profiter',
    soon: 'Bientôt',
    categories: {
      exchange: 'Exchange',
      broker: 'Broker',
      prop: 'Prop firm',
      tool: 'Outil',
      community: 'Communauté',
    },
    sectionTitles: {
      exchange: 'Exchanges crypto',
      broker: 'Brokers',
      prop: 'Prop firms',
      tool: 'Outils & ressources',
      community: 'Communauté',
    },
  },

  playerProfile: {
    eyebrow: 'Profil public',
    back: 'Retour',
    loading: 'Chargement...',
    notFound: 'Joueur introuvable.',
    viewProfile: 'Voir le profil public',
    totalPnl: 'PnL total',
    winRate: 'Winrate',
    avgRR: 'RR moyen',
    profitFactor: 'Profit factor',
    wins: 'Gagnants',
    losses: 'Perdants',
    arenas: 'Arènes',
    arenaHistory: 'Historique des arènes',
    noArenas: 'Aucune arène publique pour le moment.',
    arena: 'Arène',
    date: 'Date',
    status: 'Statut',
    rank: 'Rang',
    trades: 'Trades',
    statusLive: 'Live',
    statusUpcoming: 'À venir',
    statusEnded: 'Terminée',
  },

  payouts: {
    title: 'Payouts',
    viewCertificate: 'Voir le certificat de payout',
    download: 'Télécharger',
  },

  payoutPage: {
    title: 'Payout',
    eyebrow: 'Mes gains',
    intro: 'Retrouve ici les arènes que tu as gagnées, ta place et le montant de ta récompense. Soumets ton adresse ERC20 pour demander le versement.',
    backToArena: '← Arènes',
    empty: 'Tu n’as pas encore de payout disponible.',
    unknownArena: 'Arène',
    place: 'Place {{rank}}',
    erc20Label: 'Adresse ERC20 (Ethereum)',
    erc20Hint: 'Vérifie que ton adresse est bien sur le réseau Ethereum. Une erreur peut entraîner une perte définitive des fonds.',
    requestBtn: 'Request payout',
    statusAvailable: 'Disponible',
    statusPending: 'En attente',
    statusApproved: 'Versé',
    pendingMsg: 'Ta demande est en cours de traitement (sous 48 h ouvrées). Tu recevras un email dès que le virement sera confirmé.',
    approvedMsg: 'Ton payout a été envoyé. Merci d’avoir participé !',
    invalidAddress: 'Adresse ERC20 invalide (format 0x suivi de 40 caractères hexadécimaux).',
    invalidSession: 'Session expirée. Reconnecte-toi.',
  },

  badges: {
    title: 'Badges',
    parisChampion: 'Champion Paris 2026',
    parisChampionDesc: 'Vainqueur de la finale amateur BTF à Paris',
    parisChampionLong: 'Décerné aux vainqueurs de la compétition amateur en présentiel du Breakout Trading Fight, à Paris les 4 et 5 juin 2026. La consécration ultime de l\u2019arène.',
    champion: 'Champion Online',
    championDesc: 'A terminé 1er d\u2019une arène',
    championLong: 'Décerné aux traders qui terminent à la 1ʳᵉ place d\u2019une arène de trading. Il récompense la meilleure performance (PnL) parmi tous les participants d\u2019une compétition terminée.',
    summerChampion: 'Champion Summer',
    summerChampionDesc: '#1 du leaderboard global Summer Season',
    summerChampionLong: 'Décerné au trader #1 du leaderboard global de la Summer Season. En plus du badge, le champion remporte le maillot officiel et son invitation pour trader dans l\u2019arène physique du BTF 2027.',
    autumnChampion: 'Champion Autumn',
    autumnChampionDesc: '#1 du leaderboard global Autumn Season',
    autumnChampionLong: 'Décerné au trader #1 du leaderboard global de l\u2019Autumn Season. Le champion remporte son invitation pour trader dans l\u2019arène physique du BTF 2027.',
    btf2026: 'BTF 2026',
    btf2026Desc: 'A participé aux qualifications BTF 2026 à Paris',
    btf2026Long: 'Badge commémoratif réservé aux participants des qualifications BTF 2026 à Paris, lors de l\u2019événement physique des 4 et 5 juin. Un symbole réservé aux pionniers de l\u2019arène.',
    locked: 'À débloquer',
    unlockedLabel: 'Badge débloqué',
    lockedLabel: 'Badge verrouillé',
    howEarned: 'Comment l\u2019obtenir',
    tapHint: 'Touchez un badge pour le détail',
  },

  sections: {
    myCompetitionsEyebrow: 'Mes compétitions',
    activeArenasTitle: 'Tes arènes actives',
    emptyNoJoin: "Tu n'as encore rejoint aucune compétition.",
    emptyNoActive: "Tu n'as aucune arène active en ce moment.",
    hintChoose: 'Choisis-en une dans la liste publique ci-dessous.',
    hintHistory: 'Tes anciennes arènes sont dans l’onglet Live.',
    historyEyebrow: 'Historique',
    historyTitle: 'Arènes archivées',
    historySub: 'Toutes les arènes terminées, avec leur classement final — même celles auxquelles tu n\u2019as pas participé.',
    publicEyebrow: 'Ouverte maintenant',
    seasonEyebrow: 'Saison en cours',
    publicTitle: 'Arènes',
    publicSub: "Affronte d'autres traders. Monte dans le classement. Gagne des récompenses.",
    publicEmpty: 'Aucune compétition publique disponible pour le moment.',
  },

  process: {
    eyebrow: 'Comment ça marche',
    title: '3 étapes pour participer',
    sub: 'Le parcours doit être simple : créer son compte, rejoindre une arène, trader pour monter au classement.',
    step1Title: 'Crée un compte',
    step1Text: 'Inscris-toi rapidement avec ton email, ton pseudo et une vérification téléphone.',
    step2Title: 'Rejoins une arène',
    step2Text: "Choisis une compétition publique et entre le code donné par l'organisateur.",
    step3Title: 'Trade et gagne des prix',
    step3Text: 'Trade sur la plateforme, suis ton classement et vise les récompenses.',
  },

  platform: {
    eyebrow: 'La plateforme',
    title: 'Trade sur notre plateforme',
    sub: 'Inscris-toi à une arène, trade sur BTF Arena et gagne des prix.',
    step1: 'Inscris-toi à une arène',
    step2: 'Trade sur notre plateforme',
    step3: 'Gagne des prix',
    alt: 'Terminal de trading BTF Arena.',
    cta: 'Créer un compte',
  },

  archived: {
    ended: 'Terminée',
    leaderboard: 'Classement →',
    myRank: 'Ma position',
    myPnl: 'Mon PnL',
    participants: 'Participants',
    notJoined: 'Tu n\u2019y as pas participé',
  },

  myCard: {
    myPnl: 'Mon PnL',
    endsIn: 'Fin dans',
    statusLabel: 'Statut',
    startsIn: 'Démarre dans',
    ended: 'Terminée',
    trades: 'Trades',
    rank: 'Rang',
    arenaClosed: 'Arène fermée',
    opensIn: '🔒 Ouvre dans {{countdown}}',
    trade: 'TRADER',
    leaderboard: 'Leaderboard',
  },

  publicCard: {
    traders: 'Traders',
    registeredTraders: 'traders inscrits',
    period: 'Période',
    joined: 'Inscrit',
    youAreJoined: 'Tu es inscrit',
    arenaOpen: 'Arène ouverte',
    liveNow: 'Live maintenant',
    arenaClosed: 'Arène fermée',
    joinClosed: 'Inscriptions fermées',
    join: 'Rejoindre',
    joinArena: "Rejoindre l'arène",
    viewArena: "Voir l'arène",
    viewRules: 'Voir les règles',
    leaderboard: 'Leaderboard',
    prizeToWin: 'À gagner',
    startsIn: 'Démarre dans',
    endsIn: 'Se termine dans',
    place: '{{rank}}e',
    paperCompetition: 'Compétition de trading en Paper Trading',
    realCompetition: 'Compétition de trading en conditions réelles',
    registrationEnds: 'Inscriptions ouvertes jusqu’au',
    registrationClosed: 'Inscriptions terminées',
    tradingStarts: 'Le trading démarre le',
    dailyDrawdown: 'Drawdown journalier',
    prizePool: 'Grosse dotation',
    online: 'Arène en ligne',
    freeEntry: 'Entrée libre',
    teamChip: 'Équipe de 4',
    teams: 'Équipes',
  },

  team: {
    kicker: 'Compétition',
    title: 'Mon équipe',
    lead: 'Les équipes de 4 traders : chacun trade individuellement, le classement additionne vos PnL.',
    create: 'Créer une équipe',
    disabledHint: 'La création d’équipes est temporairement désactivée. Tu peux encore rejoindre une équipe existante avec un code.',
    namePlaceholder: 'Nom de l’équipe',
    createAction: 'Créer l’équipe',
    join: 'Rejoindre avec un code',
    joinAction: 'Rejoindre',
    saving: 'En cours…',
    inviteCode: 'Code d’invitation',
    copy: 'Copier',
    copied: 'Copié',
    inviteHint: 'Envoie ce code à 3 traders pour compléter l’équipe.',
    owner: 'Créateur',
    member: 'Membre',
    kick: 'Exclure',
    leave: 'Quitter l’équipe',
    disband: 'Dissoudre l’équipe',
    complete: 'Complète',
    incomplete: 'Incomplète',
    locked: 'L’équipe est verrouillée le temps d’une arène.',
    badgeChange: 'Badge',
    invalidImage: 'Choisis une image valide.',
    error: 'Action impossible',
  },

  joinModal: {
    eyebrow: "Rejoindre l'arène",
    instruction: "Saisis le code communiqué par l'organisateur.",
    codePlaceholder: 'CODE COMPÉTITION',
    cancel: 'Annuler',
    join: 'Rejoindre',
    joinTeam: 'Inscrire mon équipe',
    rulesTitle: "Règles de l'arène",
    dailyDrawdownRule: 'Limite de drawdown journalier de {{percent}} %',
    dailyDrawdownRuleDesc: "Si ton équité descend de {{percent}} % sous ton solde de début de journée, ton compte est éliminé et tu ne peux plus trader. Reset à 9h00 heure de Paris.",
    teamRequired: 'Rejoins une équipe existante avec un code avant de t’inscrire.',
    teamMissing: 'Il faut 4 membres pour inscrire l’équipe à cette arène.',
    teamReady: '{{name}} est complète. Tu peux l’inscrire.',
    teamOwnerOnly: 'Seul le créateur de l’équipe peut l’inscrire.',
    goToTeam: 'Ouvrir mon équipe',
  },

  leaderboard: {
    backToCompetitions: 'Retour compétitions',
    unavailable: 'Leaderboard indisponible',
    loading: 'Chargement du leaderboard...',
    paused: 'En pause',
    live: 'Live',
    updatedAt: 'maj {{time}}',
    endsIn: 'Fin dans',
    startsIn: 'Démarre dans',
    startsOn: 'Démarre le',
    endsOn: 'Termine le',
    statusLabel: 'Statut',
    ended: 'Terminée',
    participants: 'Participants',
    avgPnl: 'PnL moyen',
    dailyRule: 'Drawdown journalier',
    dailyRisk: 'Drawdown journalier max {{percent}} %',
    loadMore: 'Charger plus',
    totalTrades: 'Total trades',
    yourPosition: 'Ta position',
    livePodium: 'Podium en direct',
    top3: 'Top 3',
    fullRanking: 'Classement complet',
    allTraders: 'Tous les traders',
    noTraderYet: 'Aucun trader inscrit pour le moment.',
    noRankedYet: 'Personne a encore passé un trade dans cette arène.',
    breached: 'Éliminé',
    breachedTitle: 'Compte éliminé — limite de drawdown journalier atteinte',
    breachedList: 'Éliminés',
    breachedSectionTitle: 'Comptes éliminés',
    breachedSectionHint: 'Ces traders ont atteint la limite de drawdown journalier. Ils sont hors classement et ne peuvent plus trader.',
    enrolledList: 'Participants',
    enrolledNoTrade: 'Inscrits — pas encore tradé',
    enrolledNoTradeHint: 'Pas de rang tant qu’aucun ordre n’a été exécuté.',
    thRank: 'Rang',
    thTrader: 'Trader',
    thDivision: 'Division',
    thPnlPct: 'PnL %',
    thPnlUsd: 'PnL USD',
    thTrades: 'Trades',
    thLastUpdate: 'Dernière maj',
    podiumOnly: 'Podium uniquement — pas encore de challenger en dehors du top 3.',
    reward: 'Récompense',
    prizes: 'LOTS À GAGNER',
    toWin: 'À gagner',
    mainPrize: 'Lot principal',
    cashValue: 'Valeur cash {{amount}}',
    defaultPrizeDesc:
      'Remis aux meilleurs traders à la fin de la compétition. Plus tu grimpes dans le ranking, plus tu te rapproches du lot.',
    breakdown: 'Répartition',
    breakdownTba: 'Répartition annoncée à la fin de la compétition.',
    otherPrizes: 'Autres places',
    prizeCount: '{{count}} lot',
    prizeCount_other: '{{count}} lots',
    rankRange: '{{from}} – {{to}}',
    podium: 'Podium',
    promoOffer: 'Offre partenaire',
    promoCta: 'Découvrir l’offre',
    promoGift: 'Cadeau partenaire',
    giftTitle: 'Réservé aux Arena Traders pendant 1 semaine',
    giftReveal: 'Une surprise réservée aux traders de l’arène',
    giftDiscover: 'Découvrir mon cadeau',
    giftReserved: 'Bonus exclusif BTF Arena',
    giftPrime: 'sur vos Challenges Primes',
    giftAll: 'sur tous les Challenges',
    copyCode: 'Copier le code',
    codeCopied: 'Code copié !',
    giftUseOffer: 'Profiter de mon bonus',
    discordKicker: 'Communauté',
    discordTitle: 'Breakout',
    discordCta: '+700 Traders',
    waitingFor: 'En attente du #{{place}}',
    trades: 'trades',
    yourRanking: 'Ton classement',
    noTrade: 'Aucun trade',
    rankTier1: '1er',
    rankTier2: '2ème',
    rankTier3: '3ème',
    rankTierN: '{{rank}}ème',
    rewardAlt: 'Récompense',
    openTerminal: 'Trade dans l’Arène',
    signUp: 'S’inscrire',
    enterArena: 'Rentrer dans l’arène',
    bannerKicker: 'Cup officielle',
    arenaLive: 'Arène en direct',
    arenaEnded: 'Arène terminée',
    arenaSoon: 'Arène à venir',
    liveRanking: 'Classement en direct',
    performance: 'Performance',
    lastActivity: 'Dernière activité',
    you: 'Vous',
    lotsTotal: '{{count}} lot au total',
    lotsTotal_other: '{{count}} lots au total',
    eventOngoing: 'Événement en cours',
    liveUpdateHint: 'Les classements se mettent à jour en direct.',
    lastUpdate: 'Dernière mise à jour',
    raceSubtitle: 'Évolution du top 5 · temps réel',
    fightStarts: 'Le fight démarre dans',
    unitDays: 'Jours',
    unitHours: 'Heures',
    unitMinutes: 'Minutes',
    unitSeconds: 'Secondes',
    feedTitle: 'Flux en direct',
    feedOpened: 'vient de prendre un trade sur',
    feedClosed: 'vient de clôturer un trade sur',
    feedEmpty: 'Aucun trade pour le moment.',
  },

  settings: {
    invalidSession: 'Session invalide',
    updateFailed: 'Modification impossible',
    profileUpdated: 'Profil mis à jour',
    uploadFailed: 'Upload impossible',
    avatarUpdated: 'Photo de profil mise à jour',
    phoneNotProvided: 'Non renseigné',
    phoneVerified: 'Vérifié',
    phoneToReverify: 'À revérifier',
    backToArena: 'Retour arena',
    settings: 'Réglages',
    traderProfile: 'Profil trader',
    intro: 'Gère ton pseudo, ta photo, ton téléphone et tes réseaux visibles sur BTF Arena.',
    uploading: 'Upload...',
    changePhoto: 'Changer photo',
    information: 'Informations',
    email: 'Email',
    username: 'Pseudo',
    phone: 'Téléphone',
    phoneHelp: "Numéro vérifié à l'inscription. Il ne peut pas être modifié ici (anti multi-comptes).",
    socials: 'Réseaux sociaux',
    saving: 'Sauvegarde...',
    save: 'Sauvegarder',
    preferences: 'Préférences',
    language: 'Langue',
    languageHelp: "Choisis la langue d'affichage de la plateforme.",
  },

  terminal: {
    tradingTerminal: 'Terminal de trading',
    traderAccess: 'Accès trader',
    competitionLive: 'Compétition live',
    competitionUpcoming: 'Compétition à venir',
    paperTrading: 'Paper trading',
    balance: 'Solde',
    remaining: 'Fin',
    logout: 'Déconnexion',
    home: 'Accueil',
    leaderboard: 'Leaderboard',

    catCrypto: 'Crypto',
    catStocks: 'Actions',
    catIndices: 'Indices',
    catCommodities: 'Matières',
    catForex: 'Forex',

    searchPlaceholder: 'Rechercher BTC, Bitcoin, ETH...',
    markets: 'Marchés',
    lastPrice: 'Dernier prix',
    marketClosed: 'Marché fermé',
    noPairFound: 'Aucune paire trouvée.',

    margin: 'Marge',
    leverage: 'Levier',
    maxBuyingPower: 'Pouvoir max',
    usedByOrder: "Utilisé par l'ordre : {{amount}} USD",
    marketClosedBanner: "Marché fermé — trading indisponible hors heures d'ouverture",
    marketPrice: 'Prix du marché',
    limitPrice: 'Prix limite',
    sizeLots: 'Lots',
    sizeQty: 'Quantité',
    usdAmount: 'Montant USD',
    projectedProfit: 'Gain projeté',
    projectedLoss: 'Perte projetée',
    tpHintShort: 'TP doit être < prix de ref pour un short',
    tpHintLong: 'TP doit être > prix de ref pour un long',
    slHintShort: 'SL doit être > prix de ref pour un short',
    slHintLong: 'SL doit être < prix de ref pour un long',
    btnWaitingEvent: "En attente de l'événement",
    btnEventEnded: 'Événement terminé',
    startsIn: 'Démarre dans',
    btnMarketClosed: 'Marché fermé',
    btnEnterQty: 'Entre une quantité',
    btnSending: 'Envoi...',
    sideTotal: 'Total position',
    estimatedFees: 'Frais de trading estimés',
    marginUsed: 'Marge utilisée pour cet ordre',
    dailyDrawdownLabel: 'Limite de perte journalière ({{percent}} %)',
    dailyDrawdownValue: 'Plancher {{amount}} USD',
    dailyDrawdownHint: "Compte éliminé si ton équité atteint ce niveau aujourd'hui (reset 9h00 Paris).",
    breachedBtn: 'Compte éliminé',
    breachedBanner: 'Compte éliminé — limite de drawdown journalier atteinte. Tu ne peux plus trader dans cette arène.',

    partial: 'partiel',
    clickToEdit: 'Cliquer pour modifier',
    close: 'Fermer',
    partialClose: 'Fermeture partielle',
    closeNow: 'Fermer maintenant',
    customPct: '% personnalisé',
    quantity: 'Quantité',
    cancel: 'Annuler',
    closeFull: 'Fermer 100%',
    closePct: 'Fermer {{pct}}%',

    invalidTp: 'Take profit invalide',
    invalidSl: 'Stop loss invalide',
    slBelowCurrent: 'Le stop loss doit être sous le prix actuel',
    slAboveCurrent: 'Le stop loss doit être au-dessus du prix actuel',
    tpAboveCurrent: 'Le take profit doit être au-dessus du prix actuel',
    tpBelowCurrent: 'Le take profit doit être sous le prix actuel',
    tpAboveEntry: 'Le take profit doit être au-dessus du prix d\'entrée (gain uniquement)',
    tpBelowEntry: 'Le take profit doit être sous le prix d\'entrée (gain uniquement)',
    invalidTpQty: 'Quantité TP invalide',
    invalidSlQty: 'Quantité SL invalide',
    partialTpTooBig: 'Pour un TP partiel, la quantité doit être inférieure à la taille totale',
    partialSlTooBig: 'Pour un SL partiel, la quantité doit être inférieure à la taille totale',
    slTpError: 'Erreur SL/TP',
    buy: 'Acheter',
    sell: 'Vendre',
    tpSlTitle: 'Take profit / Stop loss',
    position: 'Position',
    entry: 'Entrée',
    market: 'Marché',
    liquidation: 'Liquidation',
    entirePosition: 'Intégralité de la position',
    partialExit: 'Sortie partielle',
    trigger: 'Déclencheur',
    entryDistance: "Distance d'entrée",
    qtyToCloseTp: 'Quantité à fermer au TP',
    qtyToCloseSl: 'Quantité à fermer au SL',
    clearAll: 'Tout effacer',
    confirm: 'Confirmer',

    tabPositions: 'Positions',
    tabOrders: 'Ordres',
    tabHistory: 'Historique',
    dragResize: 'Tirer pour agrandir ou réduire',
    collapseSection: 'Réduire la section',
    expandSection: 'Agrandir la section',
    emptyPositions: 'Aucune position. Commencez à trader avec un effet de levier ↗',
    emptyOrders: 'Aucun ordre ouvert.',
    emptyTrades: "Aucun trade pour l'instant.",
    thMarket: 'Marché',
    thPrice: 'Prix',
    thOpenQty: 'Quantité ouverte',
    thEntryPrice: "Prix de l'ouverture",
    thCurrentPrice: 'Prix actuel',
    thValue: 'Valeur',
    thLiqEstimate: 'Estimation du prix de liquidation',
    thInitialMargin: 'Marge initiale',
    thUnrealizedPnl: 'Gains et pertes non réalisés',
    thTpSl: 'TP / SL',
    thSide: 'Side',
    thType: 'Type',
    thLimitPrice: 'Prix limite',
    thQty: 'Quantité',
    thLeverage: 'Levier',
    thReserved: 'Réservé',
    thTime: 'Heure',
    thAction: 'Action',
    thFees: 'Frais',
    typeMarket: 'Marché',
    typeLimit: 'Limite',
    long: 'Long',
    short: 'Short',

    open: 'Ouvrir',
    yourPosition: 'Ta position',
    yourRanking: 'Ton classement',
    noParticipant: 'Aucun participant dans le classement.',
    noActiveTrader: 'Aucun trader actif dans le roster.',
    trades: 'trades',
    showTrades: 'Afficher les trades',
    enableSound: 'Activer le son',
    chartSettings: 'Paramètres',

    connectionFailed: 'Connexion impossible',
    demoInvalidQty: 'Quantité invalide pour la démo.',
    invalidLimitPrice: 'Prix limite invalide.',
    invalidQtyGt0: 'Quantité invalide. Entre une quantité supérieure à 0.',
    orderRejected: 'Ordre refusé',
    cancelRejected: 'Annulation refusée',
    closeRejected: 'Clôture refusée',
    limitUpdateRejected: 'Modification du prix limite refusée',
    slTpUpdateRejected: 'Modification SL/TP refusée',

    paperNotActive: "Mode paper non actif. Demande à un administrateur d'activer le paper trading.",
    noActiveCompetition: 'Aucune compétition active pour ce terminal. Retourne sur',
    noActiveCompetitionSuffix: 'pour rejoindre une arène.',
    realModeNotice: "Cette compétition est en mode réel. Le terminal réel n'est pas encore disponible dans cette version.",
    loadingTerminal: 'Chargement du terminal',
    accessRequired: 'Accès requis',
    liveTerminalTitle: 'Terminal BTF Live',
    competitionTerminalTitle: 'Terminal de compétition',
    liveAccessDesc: "Le terminal Live est réservé aux traders inscrits au roster de l'événement. Saisis ton code admin pour entrer.",
    competitionAccessDesc: 'Le terminal de trading est réservé aux joueurs inscrits à une compétition. Connecte-toi sur la BTF Arena pour rejoindre une arène.',
    enterCode: 'Saisir mon code',
    goToArena: 'Aller sur BTF Arena',
    pickArena: 'Choisis ton arène',
    pickArenaLead: 'Sélectionne une de tes arènes actives pour ouvrir le terminal.',
    switchArena: 'Changer d’arène',
    arenaLabel: 'Arène',
    arenaLive: 'Live',
    arenaSoon: 'Bientôt',
    arenaOpen: 'Ouverte',
    arenaEnded: 'Terminée',
    arenaEndsIn: 'Fin dans {{time}}',
    arenaStartsIn: 'Départ dans {{time}}',
    arenaTraders: '{{count}} traders',
    arenaCurrent: 'En cours',
    arenaOpening: 'Ouverture…',
    arenaLocked: 'Pas encore tradable',
    arenaEmpty: 'Tu n’as pas d’arènes actives pour le moment.',
    arenaJoinCta: 'Rejoindre une arène',
    arenaOpenError: 'Impossible d’ouvrir cette arène.',
    mobileOrders: 'Ordres',
    mobileChart: 'Graphique',
    mobileLeaderboard: 'Leaderboard',
  },

  sponsor: {
    sponsoredBy: 'Sponsorisé par {{name}}',
    partnerTag: 'Sponsor officiel',
    signUpShort: 'S’inscrire',
    gateTitle: 'Condition de participation',
    gateStep1: 'Crée ton compte via notre lien de parrainage.',
    gateStep2: 'Colle ton identifiant public ci-dessous pour débloquer l’accès.',
    signUp: 'S’inscrire sur {{name}}',
    idLabel: 'Identifiant public',
    idPlaceholder: 'Ton identifiant public',
    idHint: 'Tu trouves ton identifiant public dans ton compte {{name}}, rubrique Paramètres / Compte.',
    missingId: 'Saisis ton identifiant public pour rejoindre.',
    idInvalid: 'Format d’identifiant invalide. Attendu : quelque chose comme {{example}}.',
    disqualifyWarning: 'Un identifiant incorrect entraînera la disqualification.',
    introTitle: 'Compétition réservée aux utilisateurs {{name}}',
    introSubtitle: 'Cette arène est ouverte exclusivement aux traders disposant d’un compte {{name}} actif.',
    ninjatraderAbout:
      'NinjaTrader est une plateforme de trading professionnelle pour futures, forex et actions. Charts avancés, exécution rapide et outils d’analyse utilisés par des milliers de traders actifs.',
    signUpFree: 'Inscription gratuite (1 minute)',
    alreadyHaveAccount: 'J’ai déjà un compte',
    creatingAccount: 'Je crée mon compte',
    emailLabel: 'Adresse email {{name}}',
    emailPlaceholder: 'ton@email.com',
    emailHint: 'Indique l’adresse email exacte liée à ton compte {{name}}.',
    emailHintNew:
      'Une fois ton compte créé sur NinjaTrader, reviens ici et saisis la même adresse email que celle utilisée lors de l’inscription.',
    emailInvalid: 'Adresse email invalide.',
    emailVerifyWarning:
      'NinjaTrader vérifie les adresses email des participants. Une adresse incorrecte entraînera ta disqualification automatique de la compétition.',
    signUpViaAffiliate:
      'Crée ton compte NinjaTrader gratuitement via notre lien partenaire. L’inscription prend environ 1 minute.',
    afterSignUpNote:
      'Une fois ton compte créé, reviens ici, clique sur « J’ai déjà un compte » et saisis l’adresse email utilisée lors de l’inscription.',
    accountCreated: 'J’ai créé mon compte',
    continue: 'Continuer',
    back: 'Retour',
    missingEmail: 'Saisis ton adresse email {{name}} pour continuer.',
  },

  dashboardHeader: {
    eyebrow: 'Breakout Trading Fight',
    onAir: 'On Air',
    standby: 'Standby',
    timeRemaining: 'Temps restant',
    timeElapsed: 'Temps écoulé',
    tradersInArena: 'Traders en arène',
    enterFullscreen: 'Activer le plein écran',
    exitFullscreen: 'Quitter le plein écran',
    fullscreen: 'Plein écran',
  },

  footer: {
    bugReportSubject: '[BTF Arena] Rapport de bug',
    bugReportBody:
      "Décris le bug rencontré :\n\n- Page :\n- Ce qui s'est passé :\n- Ce qui était attendu :\n- Étapes pour reproduire :\n",
    cgu: 'CGU',
    privacy: 'Confidentialité',
    legalNotice: 'Mentions légales',
    risks: 'Risques',
    rules: 'Règlement',
    reportBug: 'Reporter un bug',
  },

  legal: {
    back: 'Retour à BTF Arena',
    brand: 'Breakout Trading Fight',
    cgu: {
      title: "Conditions Générales d'Utilisation",
      intro:
        "Ces Conditions Générales d'Utilisation encadrent l'accès à Breakout Trading Fight, plateforme de démonstration, de compétition et de paper trading.",
      sections: [
        {
          title: 'Objet du service',
          body:
            "Breakout Trading Fight propose des interfaces de suivi de trading, de compétition et de simulation. Sauf mention contraire explicite, les fonctionnalités de paper trading n'impliquent aucun ordre réel, aucun dépôt et aucun retrait.",
        },
        {
          title: 'Accès utilisateur',
          body:
            "L'utilisateur s'engage à fournir des informations exactes lors de son inscription et à ne pas contourner les mécanismes de vérification, notamment l'email et le téléphone lorsqu'ils sont demandés.",
        },
        {
          title: 'Comportements interdits',
          body:
            "Sont interdits : multi-comptes frauduleux, tentative d'accès non autorisé, manipulation de leaderboard, automatisation abusive, attaque du service ou usage contraire aux lois applicables.",
        },
        {
          title: 'Compétitions',
          body:
            "Les règles propres à chaque compétition, durée, classement, prix éventuel et critères de participation peuvent varier. Toute participation implique l'acceptation des règles affichées pour la compétition concernée.",
        },
        {
          title: 'Disponibilité',
          body:
            "La plateforme peut être modifiée, suspendue ou interrompue pour maintenance, évolution produit, incident technique ou raison de sécurité.",
        },
      ],
    },
    confidentialite: {
      title: 'Politique de confidentialité',
      intro:
        'Cette page explique les données susceptibles d’être utilisées pour faire fonctionner la plateforme et sécuriser les inscriptions.',
      sections: [
        {
          title: 'Données collectées',
          body:
            "Selon les fonctionnalités utilisées, la plateforme peut traiter : email, pseudo, numéro de téléphone de vérification, identifiants de session, participation aux compétitions et performances de paper trading.",
        },
        {
          title: 'Finalités',
          body:
            "Ces données servent à authentifier les utilisateurs, prévenir le multi-compte, afficher les leaderboards, sécuriser l'accès et améliorer l'expérience produit.",
        },
        {
          title: 'Services tiers',
          body:
            "Des prestataires peuvent être utilisés pour l'envoi d'emails transactionnels, la vérification SMS, l'hébergement, les analytics techniques ou l'affichage de graphiques.",
        },
        {
          title: 'Conservation',
          body:
            "Les données sont conservées pendant une durée proportionnée au fonctionnement du service, à la sécurité et aux obligations légales éventuelles.",
        },
        {
          title: 'Contact et droits',
          body:
            `Pour toute demande liée aux données personnelles, contactez ${CONTACT_EMAIL}.`,
        },
      ],
    },
    deleteAccount: {
      title: 'Supprimer un compte BTF Arena',
      intro:
        'Cette page concerne l’application et le site BTF Arena (Breakout Trading Fight by BLOCKS). Elle décrit comment demander la suppression de votre compte et des données associées.',
      sections: [
        {
          title: 'Comment demander la suppression',
          body:
            `1) Dans l’appli BTF Arena : Profil → Paramètres → Supprimer mon compte, puis confirmez. La suppression est immédiate.\n2) Par e-mail : écrivez à ${CONTACT_EMAIL} avec l’objet « Suppression de compte BTF Arena » et l’adresse e-mail du compte. Nous traitons la demande sous 30 jours.`,
        },
        {
          title: 'Données supprimées',
          body:
            'Nous supprimons : e-mail, numéro de téléphone, nom / pseudo, photo de profil, sessions de connexion, jetons push et comptes paper trading liés à votre profil. Vous pourrez vous réinscrire ensuite avec les mêmes identifiants.',
        },
        {
          title: 'Données conservées',
          body:
            'Les classements d’arènes déjà publiés restent affichés de façon anonymisée (« Compte supprimé »), pour l’intégrité de la compétition. Si un payout a déjà été versé, les pièces nécessaires aux obligations comptables ou anti-fraude peuvent être conservées jusqu’à 5 ans. Aucune donnée n’est utilisée à des fins publicitaires après suppression.',
        },
      ],
    },
    mentions: {
      title: 'Mentions légales',
      intro: 'Informations générales relatives à Breakout Trading Fight.',
      sections: [
        {
          title: 'Éditeur',
          body:
            'Breakout Trading Fight by BLOCKS. Les informations juridiques complètes de la structure éditrice pourront être complétées avant ouverture publique commerciale.',
        },
        {
          title: 'Contact',
          body: `Email de contact : ${CONTACT_EMAIL}.`,
        },
        {
          title: 'Hébergement',
          body:
            "L'hébergement peut être assuré par les prestataires techniques utilisés pour le frontend, le backend, les bases de données et les services associés.",
        },
        {
          title: 'Propriété intellectuelle',
          body:
            "Les interfaces, textes, éléments graphiques, marques, logos et contenus propres à Breakout Trading Fight ne peuvent pas être réutilisés sans autorisation.",
        },
      ],
    },
    risques: {
      title: 'Avertissement sur les risques',
      intro:
        "Le trading comporte des risques importants. Cette plateforme peut afficher des performances simulées ou réelles selon le mode utilisé.",
      sections: [
        {
          title: 'Paper trading',
          body:
            "Les résultats en paper trading sont simulés. Ils ne constituent pas une performance réelle et ne garantissent aucun résultat futur.",
        },
        {
          title: 'Marchés financiers et crypto-actifs',
          body:
            "Les crypto-actifs et produits financiers peuvent être très volatils. Une perte partielle ou totale du capital est possible dans un environnement de trading réel.",
        },
        {
          title: 'Absence de conseil financier',
          body:
            "Les informations affichées sur la plateforme ne constituent ni un conseil en investissement, ni une recommandation d'achat ou de vente.",
        },
        {
          title: 'Responsabilité utilisateur',
          body:
            "Chaque utilisateur reste responsable de ses décisions, de sa gestion du risque et du respect de la réglementation applicable dans son pays.",
        },
      ],
    },
    reglement: {
      title: 'Règlement général des compétitions',
      intro:
        "Les compétitions organisées sur la plateforme BTF Arena ont pour objectif de mettre en avant les compétences des participants en matière d'analyse et de prise de décision sur les marchés financiers à travers un environnement de simulation (paper trading). Les compétitions sont gratuites et ne nécessitent aucun investissement financier de la part des participants.",
      sections: [
        {
          title: '1. Objet',
          body:
            "Les compétitions organisées sur la plateforme BTF Arena ont pour objectif de mettre en avant les compétences des participants en matière d'analyse et de prise de décision sur les marchés financiers à travers un environnement de simulation (paper trading). Les compétitions sont gratuites et ne nécessitent aucun investissement financier de la part des participants.",
        },
        {
          title: '2. Conditions de participation',
          body:
            "La participation est ouverte à toute personne majeure disposant d'un compte utilisateur valide sur la plateforme BTF Arena. L'organisateur se réserve le droit de refuser ou d'annuler la participation de tout utilisateur ne respectant pas le présent règlement ou ayant adopté un comportement frauduleux, abusif ou contraire à l'esprit de la compétition.",
        },
        {
          title: '3. Pays éligibles',
          body:
            "Certaines compétitions peuvent être limitées à des territoires spécifiques pour des raisons réglementaires. L'organisateur se réserve le droit d'exclure certains pays ou juridictions dans lesquels les compétitions, récompenses ou activités proposées pourraient être soumises à des restrictions légales particulières. Il appartient à chaque participant de vérifier qu'il est autorisé à participer conformément aux lois applicables dans son pays de résidence.",
        },
        {
          title: '4. Fonctionnement des compétitions',
          body:
            "Les compétitions se déroulent exclusivement sur des comptes de démonstration (paper trading) et n'impliquent aucun dépôt, investissement ou risque financier réel. Les classements sont établis selon les critères définis pour chaque compétition (performance, rendement, score ou tout autre indicateur communiqué avant le début de l'événement). L'organisateur se réserve le droit de modifier, suspendre ou annuler une compétition en cas de problème technique, de fraude ou de circonstances exceptionnelles.",
        },
        {
          title: '5. Récompenses',
          body:
            "Les récompenses annoncées pour chaque compétition sont attribuées aux participants éligibles selon le classement final officiel publié par l'organisateur. Les récompenses peuvent prendre la forme de sommes d'argent, de cartes cadeaux, de produits, de services, de crédits promotionnels, d'évaluations de prop firms ou de tout autre lot communiqué avant le début de la compétition. L'organisateur se réserve le droit de remplacer un lot par un autre de valeur équivalente en cas d'indisponibilité.",
        },
        {
          title: '6. Vérification des gagnants',
          body:
            "Avant l'attribution d'une récompense, l'organisateur peut demander aux gagnants de fournir des informations complémentaires permettant de vérifier leur identité et leur éligibilité. Tout participant ayant utilisé des méthodes frauduleuses, automatisées ou contraires aux règles de la compétition pourra être disqualifié sans préavis.",
        },
        {
          title: '7. Fiscalité',
          body:
            "Les gains, récompenses ou avantages obtenus dans le cadre des compétitions peuvent être soumis à des obligations fiscales selon le pays de résidence du participant. Chaque gagnant demeure seul responsable de la déclaration et du paiement de toute taxe, impôt ou contribution éventuellement applicable à la réception de ses gains. L'organisateur ne fournit aucun conseil fiscal et ne saurait être tenu responsable des obligations fiscales des participants.",
        },
        {
          title: '8. Limitation de responsabilité',
          body:
            "L'organisateur ne garantit aucun gain financier futur ni aucune performance réelle sur les marchés financiers. Les compétitions sont organisées à des fins éducatives, ludiques et de démonstration uniquement. La participation à une compétition ne constitue ni un conseil en investissement, ni une recommandation financière, ni une offre de services d'investissement.",
        },
        {
          title: '9. Acceptation du règlement',
          body:
            "Toute participation à une compétition organisée sur BTF Arena implique l'acceptation pleine et entière du présent règlement ainsi que des Conditions Générales d'Utilisation de la plateforme.",
        },
      ],
    },
  },

  rating: {
    navLabel: 'Rang',
    seoTitle: 'BTF Rating — de Bronze à Legend',
    seoDesc: 'Deux classements distincts : le PnL saisonnier dont le numéro 1 se qualifie pour Paris, et le BTF Rating permanent de Bronze à Legend.',
    kicker: 'BTF Rating',
    myDivision: 'Ma division',
    points: '{{points}} pts',
    worldRankLabel: 'Rang mondial',
    worldRank: 'Rang mondial #{{rank}} / {{total}}',
    nextAt: 'Prochaine division : {{label}} dans {{points}} pts',
    maxDivision: 'Division maximale — défends ton trône',
    you: 'Toi',
    ladderKicker: 'Divisions',
    ladderTitle: 'De Bronze à Legend',
    ladderLead: "Tes résultats en arène te font gagner ou perdre des Arena Points. Ta division reflète ton vrai niveau — pas ton volume de trades.",
    ladderIntro: 'Grimpe dans le classement et collecte les badges',
    worldKicker: 'BTF Rating · Permanent · Non qualificatif',
    worldTitle: 'Classement de niveau permanent',
    worldLead: 'Arena Points cumulés sur toutes les saisons. Ce classement détermine Bronze → Legend, pas les invitations pour Paris.',
    seasonCta: 'Classement de saison',
    emptyWorld: "Aucun joueur classé pour le moment. Termine une arène pour entrer au classement.",
    loadMore: 'Charger plus',
    boardLabel: 'Classement',
    unavailable: 'Classement indisponible pour le moment.',
    tabSeason: 'Saison',
    tabRating: 'BTF Rating',
    tabHint: 'Saison = n°1 invité à Paris. BTF Rating = ton niveau, sans qualification.',
    parisLeadMobile: 'Le n°1 du classement de saison est invité au Major de Paris.',
    parisKicker: 'Saison · N°1 → Paris',
    parisTitle: 'Le champion de la saison va à Paris',
    parisLead: "Le classement de saison cumule ton PnL sur toutes les arènes hebdomadaires. À la clôture, le numéro 1 gagne son invitation pour trader en live au BTF Major de Paris, sur scène, devant le public.",
    parisStepOne: 'Participe aux arènes hebdo',
    parisStepTwo: 'Monte au classement de saison',
    parisStepThree: 'Termine n°1 → Paris',
    seasonalBadge: 'Classement saisonnier',
    qualifyingKicker: 'Qualification officielle pour Paris',
    currentSeason: 'Saison en cours',
    seasonComing: 'Ça arrive',
    top1Paris: 'N°1 → Paris',
    seasonEndsIn: 'Fin dans',
    seasonEnded: 'Saison terminée',
    parisBound: 'Part à Paris',
    parisBoundLead: 'Compétite dans l’arène du Major de Paris',
    parisQualified: 'Qualifié Paris',
    arenasShort: 'arènes',
    emptySeason: 'Le classement de cette saison apparaîtra après les premières arènes.',
    seasonPrizesLabel: 'Les lots à gagner',
    seasonPrizeBadge: 'Badge champion',
    seasonPrizeShirt: 'Tee-shirt officiel',
    seasonPrizeParis: 'Invitation Major Paris 2027',
    fullSeasonRanking: 'Voir le classement complet de la saison',
    yourSeasonPosition: 'Ta position',
    permanentBadge: 'Permanent',
    permanentTitle: 'BTF Rating · Bronze à Legend',
    permanentNotQualifying: 'Mesure ton niveau. Ne qualifie pas pour Paris.',
    rulesTitle: 'Comment gagner des points ?',
    rulesFirst: '1er',
    rulesSecond: '2e',
    rulesThird: '3e',
    rulesTop10: 'Top 10%',
    rulesTop25: 'Top 25%',
    rulesTop50: 'Top 50%',
    rulesBottom: 'Bottom 50%',
    rulesEliminated: 'Éliminé',
    purposeProgressTitle: 'Progression permanente',
    purposeProgressLead: 'Tu gagnes ou perds des Arena Points après chaque arène, sans remise à zéro entre les saisons.',
    purposeIdentityTitle: 'Identité de joueur',
    purposeIdentityLead: 'Ton badge Bronze → Legend affiche immédiatement ton niveau sur ton profil et dans l’arène.',
    purposeWorldTitle: 'Comparaison mondiale',
    purposeWorldLead: 'Compare ta régularité avec tous les traders dans le classement BTF Rating permanent.',
  },

  spectateHud: {
    feed: 'Direct de l’arène',
    faceoff: 'Duel en tête',
    best: 'Meilleur PnL',
  },

  raceChart: {
    kicker: 'Course en direct',
    kickerEnded: 'Course de l’arène',
    title: 'PnL Race',
    collecting: 'Collecte des données en cours…',
    dominates: "{{name}} domine l'arène",
    momentLeader: '{{name}} prend la tête',
    momentTop3: '{{name}} entre dans le top 3',
  },

  arenaChat: {
    kicker: "Chat de l'arène",
    open: 'Ouvrir le chat',
    collapse: 'Réduire',
    tab: 'Chat arène',
    unread: '{{count}} non lu(s)',
    notice: 'Participants et spectateurs échangent ici en direct.',
    noticeMobile: 'Appuie longtemps sur un message pour répondre.',
    reply: 'Répondre',
    replyTo: 'Réponse à {{name}}',
    cancelReply: 'Annuler la réponse',
    emptyTitle: 'Le chat est ouvert',
    emptyLead: 'Sois le premier à lancer la discussion.',
    loginToWrite: 'Connecte-toi pour écrire',
    placeholder: 'Écris un message…',
    attach: 'Partager une photo',
    photo: 'Photo partagée',
    unavailable: 'Chat indisponible',
    uploadFailed: "L'envoi de la photo a échoué",
    sendFailed: "L'envoi du message a échoué",
  },

  home: {
    greetingMorning: 'Bonjour',
    greetingEvening: 'Bonsoir',
    openNews: 'Ouvrir les actualités',
    openProfile: 'Ouvrir mon profil',
    pnlGlobal: 'PNL GLOBAL',
    profitFactor: 'PROFIT FACTOR',
    worldRank: 'RANG MONDIAL',
    permanent: 'Permanent',
    toNext: '{{points}} pts vers {{label}}',
    maxDivision: 'Division max',
    competitions: 'COMPÉTITIONS',
    current: 'En cours',
    openForJoin: 'Inscriptions ouvertes',
    upcomingArenas: 'À venir',
    startsIn: 'Commence dans',
    guestEyebrow: 'Compétition de trading',
    guestTitle: 'Entre dans',
    guestTitleEm: 'l’arène.',
    guestLead: 'Inscris-toi à une arène, trade sur notre plateforme et gagne des prix.',
    guestGlobal: 'Voir le classement de saison',
  },

  account: {
    journalHint: 'Tous tes trades fermés',
    payoutsHint: 'Lots et demandes de paiement',
    settingsHint: 'Avatar, nom et téléphone',
    bonusHint: 'Offres partenaires et codes promo',
    languageHint: 'Langue de l’interface',
  },
};
