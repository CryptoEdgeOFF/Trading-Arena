...
import { formatDHMS, formatPnl, formatTime, timeAgo } from '../utils/formatters';
...
function TopBar({
  ...
  return (
    <header className="relative flex shrink-0 flex-wrap items-center gap-x-2 gap-y-1.5 border border-[#808080] bg-[#c0c0c0] px-2 py-1.5 md:px-3 md:py-1.5">
      ...
      <div className="order-4 grid w-full grid-cols-3 overflow-hidden border border-[#808080] bg-[#ececec] md:order-1 md:mr-auto md:w-auto md:min-w-[360px]">
    ...
    <div className={`relative z-40 ${className}`}>
      ...
      <div className="relative z-40">
        <button
          ...
          className="flex w-full cursor-pointer items-center gap-2 px-2 py-1 text-left text-[13px]"
        >
...
function OrderForm(props: OrderFormProps) {
  ...
  return (
    <section className="flex w-full min-w-0 shrink-0 flex-col overflow-hidden border border-[#808080] bg-[#d4d0c8] lg:w-[470px]">
      <div className="flex-1 overflow-y-auto px-3 pb-3 pt-3">
        <div className="mb-2 flex items-center gap-2">
          ...
          />
          <img
            src="/assets/pictures/kraken-logo-white.webp"
            alt="Kraken"
        ...
        {/* Buy / Sell */}
        <div className="grid grid-cols-[1fr_1fr_1.35fr] gap-1 border border-[#808080] bg-[#c0c0c0] p-1">
          <button
            type="button"
            onClick={() => setSide('long')}
            className="h-8 cursor-pointer text-[13px]"
            style={{ fontWeight: !isSell ? 'bold' : 'normal' }}
          >
            Buy
          </button>
          <button
          ...
          <div className="relative">
            <select
              ...
              className="h-8 w-full cursor-pointer pl-2 pr-6 text-[13px] outline-none"
            >
        ...
        <button
          ...
          className="mt-3 flex h-9 w-full cursor-pointer items-center justify-center text-[13px] disabled:cursor-not-allowed disabled:opacity-50"
        >
          {breached
...
function ChartArea({
  ...
  const pendingOrders = (player?.openOrders ?? [])
    ...
    .map((order) => ({
      ...
      size: Math.max(0, order.size - (order.filledSize ?? 0)),
      status: order.status,
  ...
  return (
    <section className="relative flex min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-[#2a2236] bg-[#10091c]">
      ...
      <div className="relative flex min-h-0 flex-1 items-center justify-center border border-[#808080] bg-[#ececec]">
        <div className="text-center text-[#606060]">
          <div className="text-[14px] font-bold">Chart en attente</div>
          <div className="mt-1 text-[12px]">(graphique non disponible dans cette version alpha)</div>
        </div>
      </div>
    </section>
  );
            ...
            <table className="w-full text-left text-[11.5px]">
              ...
              <tbody className="divide-y divide-[#1c181a] text-[#e0e2ea]">
                {orders.map((order) => {
                  ...
                  const filledSizeDisplay = formatEngineSizeDisplay(order.pair, filledSize, pairBase(order.pair));
                  const remainingSizeDisplay = formatEngineSizeDisplay(order.pair, remainingSize, pairBase(order.pair));
                  const latestFill = allTrades
                  ...
                  <tr key={order.id} className="hover:bg-[#181517]">
                    ...
                    <Td>
                      <div className="flex items-center gap-1.5">
                        <span>{filledSizeDisplay.text} / {orderSizeDisplay.text}</span>
                        <span className="text-[10px] text-[#7a8090]">{orderSizeDisplay.unit}</span>
                        {isPartial && (
                          <span className="rounded border border-amber-400/30 bg-amber-400/10 px-1.5 py-0.5 text-[9px] font-semibold text-amber-300">Partiel</span>
                        )}
                      </div>
                      <div className="mt-0.5 text-[9px] text-[#7a8090]">
                        Restant {remainingSizeDisplay.text} {remainingSizeDisplay.unit}
                      </div>
                    </Td>
                    ...
                    <Td className="text-right">
                      <div className="flex justify-end gap-1">
                        {latestFill && (
                          <button type="button" onClick={() => onShowFill(latestFill)} className="cursor-pointer rounded border border-violet-400/20 px-2 py-1 text-[10px] text-violet-200 hover:bg-violet-400/[0.08]">
                            Détails
                          </button>
                        )}
...
function formatCountdown(target: number, dayUnit = 'd'): string {
  return formatDHMS(target - Date.now(), dayUnit);
}
...
function CompetitionBanner({ ctx }: { ctx: { id: string; title: string; mode: 'paper' | 'real'; startAt?: number; endAt?: number; status?: 'registration' | 'starting_soon' | 'live' | 'ended'; rank?: number | null; participants?: number; pnlPercent?: number } }) {
  ...
  const countdown = target ? formatCountdown(target, i18n.language.startsWith('fr') ? 'j' : 'd') : null;
...
export default function ExchangeTerminalRedesign({ demoMode = false }: ExchangeTerminalProps) {
  const { t } = useTranslation();
  ...
  const showFillDetails = useCallback((trade: Trade) => {
  ...
  return (
    <div
      className="terminal terminal-alpha flex h-[100dvh] min-h-[100dvh] flex-col overflow-hidden text-[13px] text-black"
      style={{
    ...
    >
      ...
      <style>{`
        ...
        .terminal-alpha button,
        .terminal-alpha select,
        .terminal-alpha input {
          background: #ececec !important;
          color: #000 !important;
          border: 1px solid #808080 !important;
          font-weight: normal !important;
        }
        .terminal-alpha button:hover { background: #dcdcdc !important; }
      `}</style>
      {!livePaperMode && (
