...
import { formatDHMS, formatPnl, timeAgo } from '../utils/formatters';
...
function TopBar({
  ...
  return (
    <header className="relative grid shrink-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-1.5 gap-y-1.5 rounded-2xl border border-[#2a2236] bg-[#0b0711]/95 px-2 py-1.5 [text-size-adjust:100%] shadow-[0_18px_60px_-45px_rgba(220,38,38,0.8)] backdrop-blur md:flex md:flex-wrap md:gap-x-2 md:px-3 md:py-1.5">
      ...
      <div className="col-span-3 min-w-0 md:order-1 md:mr-auto md:w-auto">
        <div className={`grid w-full overflow-hidden rounded-xl border border-[#241e30] bg-[#15121f] ${hasDdLimit ? 'grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto_minmax(7.5rem,1.15fr)] md:min-w-[440px]' : 'grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] md:min-w-[360px]'}`}>
          ...
          <div className="border-r border-[#241e30] px-1.5 py-1 text-center md:px-2 md:py-1.5">
          ...
          {hasDdLimit && (
            <div className={`min-w-0 px-2 py-1 md:px-2.5 md:py-1.5 ${ddUrgent ? 'bg-red-500/10' : ''}`}>
              <div className="flex items-baseline justify-between gap-1">
                <span className="truncate text-[9px] uppercase tracking-[0.14em] text-[#fca5a5]">{t('terminal.equityFloorShort')}</span>
                <span className="num shrink-0 text-[12px] font-semibold text-[#fca5a5] md:text-[13px]">{fmt(dailyLimitEquity, 0)}</span>
              </div>
              {ddSafeRatio != null && (
                <div className="mt-1 h-1 overflow-hidden rounded-full bg-[#3a2230]">
                  <div
...
export default function ExchangeTerminal({ demoMode = false }: ExchangeTerminalProps) {
  ...
  const showArenaPicker = !demoMode && !liveMode && !session && !bootstrapping;
  const showCompeteBoot = !demoMode && !liveMode && !session && bootstrapping;
  ...
  return (
    ...
    >
      {showArenaPicker && (
        <>
          <CompeteHeader />
          <div className="min-h-0 flex-1 overflow-y-auto bg-[#050507]">
      ...
      {(showCompeteBoot || (livePaperMode && !session && bootstrapping)) && (
      ...
      {liveMode && livePaperMode && !session && !bootstrapping && (
        <div className="flex flex-1 items-center justify-center p-6">
          <div className="w-full max-w-md rounded-2xl border border-[#231f22] bg-[#0e0c0d] p-7 shadow-2xl shadow-black/40">
            ...
            <h2 className="font-rajdhani text-3xl font-bold text-white">
              {t('terminal.liveTerminalTitle')}
            </h2>
            <p className="mt-3 text-[13px] text-[#9498a4]">
              {t('terminal.liveAccessDesc')}
            </p>
