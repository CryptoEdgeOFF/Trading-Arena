...
import { DIVISION_COLORS, DivisionBadge, RatingCard, divisionDisplayName, type PlayerRating } from './playerRating';
...
interface PlayerProfile {
  user: { id: string; name: string; avatarUrl: string | null; country?: string | null; socials?: PlayerSocials };
  badges: UserBadge[];
  totalPnlUsd: number;
  arenas: PlayerArena[];
              ...
              >
                ...
                <div className="relative flex flex-col items-center gap-5 text-center sm:flex-row sm:text-left">
                  ...
                  <div className="min-w-0">
                    ...
                    <h1 className="display mt-1 flex flex-wrap items-center justify-center gap-2 text-2xl font-bold text-white sm:justify-start md:text-3xl">
                      {profile.user.country && (
                        <span className="text-[28px] leading-none" title={profile.user.country} aria-label={profile.user.country}>
                          {countryFlag(profile.user.country)}
                        </span>
                      )}
                      {profile.user.name}
                  ...
                  {profile.rating && (
                    <Link
                      to="/compete/rank"
                      className="flex shrink-0 flex-col items-center gap-1.5 rounded-2xl border px-5 py-4 transition-transform hover:-translate-y-0.5 sm:ml-auto"
                      style={{
                        borderColor: `${DIVISION_COLORS[profile.rating.division.id] || '#cbd5e1'}55`,
                        background: `radial-gradient(80% 80% at 50% 0%, ${DIVISION_COLORS[profile.rating.division.id] || '#cbd5e1'}18, transparent 70%)`,
                      }}
                    >
                      <DivisionBadge division={profile.rating.division} size={112} />
                      <span className="display text-sm font-black uppercase text-white">
                        {divisionDisplayName(profile.rating.division)}
                      </span>
                      <span className="num text-[10px] text-[#a1a1aa]">
                        {t('rating.points', { points: profile.rating.points })}
                        {profile.rating.worldRank != null && ` · #${profile.rating.worldRank}`}
                      </span>
                    </Link>
                  )}
                </div>
